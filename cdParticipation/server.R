...
server <- function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    min <- min(cd[[selected_var]], na.rm = TRUE)
    max <- max(cd[[selected_var]], na.rm = TRUE)
    pal <- colorNumeric(palette = brewer.pal(7, "YlGnBu"), domain = c(0, 1))
    ques = (cd[[selected_var]] - min) / (max - min)
    ...
    leaflet(cd) %>%
      ...
      addPolygons(
        ...
        fillColor = ~pal(ques),
        weight = 1,
        label = lapply(seq_len(nrow(cd)), function(i) {
          paste0(
            "<b>Congressional District:</b> ", cd$CD[i], # Add the CD label here
            "<br>",
            "<b>Participation Score:</b> ", format(round(cd[[selected_var]][i], 2), big.mark = ","),
            "<br>") %>%
            htmltools::HTML()
  ...
  # This pulls the User's cd
  intersecting_geom_reactive <- reactiveVal(5)
  ...
  observeEvent(input$map_marker_dragend, {
    rd <- state_boundary(shapefile = cd,
    ...
    if(nrow(rd) == 0){
      ...
      intersecting_geom <- cd[st_intersects(cd, st_sfc(st_point(c(input$map_marker_dragend$lng, input$map_marker_dragend$lat)), crs = st_crs(cd))) %>% lengths > 0, ]

      if (nrow(intersecting_geom) > 0) {
        # Print the geometry type (assuming 'shape_geom' contains geometry type information)
        cat("Marker is within cd type:", intersecting_geom$CD, "\n")
  ...
  observeEvent(input$map_shape_click, {
    ...
    # Find the intersecting geometry
    intersecting_geom <- cd[which(st_intersects(cd, st_sfc(st_point(c(input$map_shape_click$lng, input$map_shape_click$lat)), crs = st_crs(cd))) %>% lengths > 0), ]
  ...
  output$hist <- renderPlot({
    ...
    # Calculate the mean for the selected cd and variable
    mean <- cd %>%
      filter(CD == intersecting_geom_reactive()) %>%
    ...
    filtered_data <- cd %>% select(input$variable)
    print(dim(filtered_data))

    filtered_data %>%
      ggplot( aes(x = .data[[input$variable]])) +
      geom_histogram(fill = oasis, color = "white", bins = 5,
      ) +
      ...
      annotate("text", x = mean, y = Inf, label = paste0( "CD:", intersecting_geom_reactive(), " \nScore (Dashed)"),
      ...
      labs(
        title = "",
        x = "Voting Score",
        y = "Number of Congressional Districts"
      )
  ...
  output$demographics <- renderUI({
    geom_data <- cd %>%  filter(CD == intersecting_geom_reactive())
  ...
  output$economics <- renderUI({
    ...
    if (!is.null(geom_data)) {
      ...
      formatted_text <- paste(
        ...
        #  "<b style='font-size: 1.2em; padding-bottom: 10px;'>Legislative District:</b><br>", # Increase font size and add bottom padding
        "<b>Poverty Rate:</b><br>",
        "<b>Median Househocd Income:</b><br>",
        "<b>Median Age:</b><br>",
        "</div>",
        "<div style='text-align: right;'>",
        #          geom_data$cd, "<br>",
        round(geom_data$poverty_rate * 100, 2), "%<br>",
        "$",format(geom_data$median_househocd_income, big.mark = ","), "<br>",
        round(geom_data$median_age, 2), "years<br>",
        "</div>",
        "</div>"
      )
  ...
  output$cd <- renderUI({
    h3(paste("Congressional District", intersecting_geom_reactive()))
  })
